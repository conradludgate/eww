...
impl Spanned for ParseError {
    fn span(&self) -> Span {
        match &self.source {
            ...
            lalrpop_util::ParseError::UnrecognizedEOF { location, .. } => Span(*location, *location, self.file_id),
            lalrpop_util::ParseError::UnrecognizedToken { token, .. } => Span(token.0, token.2, self.file_id),
